...
use Account;
use App\Contracts\Auth\Permission;
    ...
    public function registerPermissions(): bool
    {
        app(Gate::class)->before(function (Authorizable $user, string $ability) {
            if (method_exists($user, 'checkPermissionTo')) {
                $guardName = Account::group($user)->guardName();

                if ($user->checkPermissionTo($ability, $guardName)) {
                    return true;
                }
            }
        });

        app(Gate::class)->after(function (Authorizable $user, string $ability) {
            $guardName = Account::group($user)->guardName();

            if (!$this->permissionClass::whereGuard($guardName)->exists()) {
                return true;
            }
        });
