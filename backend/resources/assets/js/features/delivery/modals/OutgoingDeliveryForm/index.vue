<template>
    <ModalLayout :title="$attrs.modal.options.title" :id="$attrs.modal.id">
        <v-subheader class="px-4">{{ $t('labels.contractor') }}</v-subheader>

        <v-container grid-list-lg fluid class="py-0">
            <AddressbookSelector
                v-model="delivery.contractor"
                :label="$t('hints.choose-from-addressbook')"
                :errors="$deliveries.$errors.get('contractor', [])"
            />

            <ContractorInformation
                v-if="delivery.contractor"
                :contractor="delivery.contractor"
            />
        </v-container>
        ...
        <v-container grid-list-lg fluid class="py-0">
            ...
            <v-layout row>
                ...
                <v-flex xs6>
                    <TimePicker
                        v-model="delivery.scheduleTime"
                        :label="$t('labels.develiery-time')"
                        :errors="$deliveries.$errors.get('contractor_id', [])"
                    />
                </v-flex>
            </v-layout>
...
export default {
    name: 'DeliveryOutgoingFormModal',
