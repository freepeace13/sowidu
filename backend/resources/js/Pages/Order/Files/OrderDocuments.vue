    ...
    <div class="fill-height tw-w-full">
        ...
        >
            ...
            >
                ...
                <v-toolbar-title>
                    {{ $t('order.labels.order-documents') }}
                </v-toolbar-title>
        ...
        <JumboUploadButton
            ...
            :title="$t('order.labels.add-document')"
...
export default {
    ...
    methods: {
        attachDocument({ uuid: media }) {
            const order = this.order
            this.$inertia.post(
                this.$route('orders.show.files.documents.store', {
                    order,
                }),
                {
                    media,
                },
                {
                    ...
                    only: ['errors', 'documents'],
                    preserveScroll: true,
                    preserveState: true,
                    onSuccess: () => {
                        this.$root.$emit(
                            'flash.success',
                            'Document has been attached to this order.',
                        )
                    },
                },
            )
        ...
        removeFile(media) {
            ...
            this.$confirm.ask({
                title: 'Delete',
                question: 'Do you want to delete this document?',
                type: 'delete',
                confirm: () => {
                    this.$inertia.delete(
                        this.$route('orders.show.files.documents.destroy', {
                            order,
                            media: media.uuid,
                        }),
                        {
                            ...
                            onSuccess: () => {
                                this.$root.$emit(
                                    'flash.success',
                                    'Document has been deleted and detached.',
                                )
                            },
                        },
                    )
        ...
        detachToOrder(media) {
            ...
            this.$confirm.ask({
                title: 'Detach',
                question:
                    'Do you want to detach this document from this order?',
                type: 'delete',
                confirm: () => {
                    this.$inertia.delete(
                        this.$route('orders.show.files.documents.detach', {
                            order,
                            media: media.uuid,
                        }),
                        {
                            ...
                            onSuccess: () => {
                                this.$root.$emit(
                                    'flash.success',
                                    'Document has been detached to this order.',
                                )
                            },
                        },
                    )
