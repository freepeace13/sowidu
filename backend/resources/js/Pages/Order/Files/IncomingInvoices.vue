    ...
    <div class="fill-height tw-w-full">
        ...
        >
            ...
            >
                ...
                <v-toolbar-title>
                    {{ $t('order.labels.incoming-invoices') }}
                </v-toolbar-title>
        ...
        <JumboUploadButton
            ...
            :title="$t('order.labels.add-incoming-invoice')"
...
export default {
    ...
    methods: {
        attachInvoice({ uuid: media }) {
            ...
            this.$inertia.post(
                this.$route('orders.show.files.incoming_invoices.store', {
                    order,
                }),
                {
                    media,
                },
                {
                    ...
                    onSuccess: () => {
                        this.$root.$emit(
                            'flash.success',
                            'Incoming invoice has been attached to this order.',
                        )
                    },
                },
            )
        ...
        removeFile(media) {
            ...
            this.$confirm.ask({
                title: 'Delete',
                question: 'Do you want to delete this incoming invoice?',
                type: 'delete',
                confirm: () => {
                    this.$inertia.delete(
                        this.$route(
                            'orders.show.files.incoming_invoices.destroy',
                            {
                                order,
                                media: media.uuid,
                            },
                        ),
                        {
                            ...
                            onSuccess: () => {
                                this.$root.$emit(
                                    'flash.success',
                                    'Incoming invoice has been deleted and detached.',
                                )
                            },
                        },
                    )
        ...
        detachToOrder(media) {
            ...
            this.$confirm.ask({
                title: 'Detach',
                question:
                    'Do you want to detach this incoming invoice from this order?',
                type: 'delete',
                confirm: () => {
                    this.$inertia.delete(
                        this.$route(
                            'orders.show.files.incoming_invoices.detach',
                            {
                                order,
                                media: media.uuid,
                            },
                        ),
                        {
                            ...
                            onSuccess: () => {
                                this.$root.$emit(
                                    'flash.success',
                                    'Invoice has been detached to this order.',
                                )
                            },
                        },
                    )
