    ...
    <v-app class="!tw-bg-bg-grey">
        <template v-if="!user.locked">
            ...
            >
                ...
                <!-- TEMP FIX: For modularization of the app -->
                <a
                    class="v-toolbar__title tw-mr-20 white--text hidden-xs-only"
                    href="/"
                >
...
import { Head as HeadTitle, router } from '@inertiajs/vue2'
import BulkExportProgress from '~Invoicify/Components/BulkExportProgress.vue'
...
export default {
    ...
    methods: {
        ...
        listeningToChat(user) {
            if (!this.$route().current('chatly.*')) return
