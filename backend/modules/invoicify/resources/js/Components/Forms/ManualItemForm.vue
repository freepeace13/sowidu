...
const isCreating = computed(() => !currentItem.value)
const loadingText = computed(() =>
    isCreating.value ? $t('buttons.creating') : $t('buttons.updating'),
)
...
function show(item = null) {
    ...
    if (item) {
        ...
        const match = itemTypes.value.find((i) => i.value === item.type.id)
...
function submit() {
    ...
    let route = $route('invoicify.manual_items.store', { invoice })

    if (!isCreating.value) {
        route = $route('invoicify.manual_items.update', {
